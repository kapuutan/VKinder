            ...
            for user in users:
                ...
                if not top_photos:
                    ...
                    for photo in top_photos:
                        photo_attachment = vk_api.VkUpload(vk).photo_messages(photo)[0]['id']
                        vk.messages.send(
                            user_id=user_id,
                            message=message,
                            attachment=f'photo{photo_attachment}',
                            random_id=random.randint(1, 999999)
                        )
                        time.sleep(3)

                    print(f'Сообщения отправлены пользователю {user_id} с идентификатором {user_info.get("id")}')
                else:
